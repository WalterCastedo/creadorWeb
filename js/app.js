...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Horarios Modo Switcher (Rangos vs Diario)
  ...
  function renderGaleriaInputs() {
    ...
    galeriaState.forEach((item, idx) => {
      ...
      div.innerHTML = `
        <input type="text" data-idx="${idx}" value="${item.url}" class="galeria-input flex-1 bg-slate-900 border border-slate-700 rounded-lg px-2.5 py-1 text-xs text-white">
  ...
  function renderTestimoniosInputs() {
    ...
    testimoniosState.forEach((item, idx) => {
      const div = document.createElement('div');
      div.className = 'bg-slate-900 p-2.5 rounded-lg border border-slate-700 space-y-1.5 relative';
      div.innerHTML = `
        ...
        <input type="text" data-idx="${idx}" data-field="cliente" value="${item.cliente}" placeholder="Nombre del Cliente" class="testimonio-input w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white">
        <textarea data-idx="${idx}" data-field="texto" rows="1" placeholder="Opinión" class="testimonio-input w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white">${item.texto}</textarea>
      `;
      container.appendChild(div);
    });
  ...
  function renderFaqInputs() {
    ...
    faqState.forEach((item, idx) => {
      ...
      div.innerHTML = `
        ...
        <input type="text" data-idx="${idx}" data-field="pregunta" value="${item.pregunta}" placeholder="Pregunta" class="faq-input w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white">
        <textarea data-idx="${idx}" data-field="respuesta" rows="1" placeholder="Respuesta" class="faq-input w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white">${item.respuesta}</textarea>
      `;
      container.appendChild(div);
    });
  ...
  function renderServicesInputs() {
    ...
    serviciosState.forEach((serv, index) => {
      ...
      card.className = 'bg-slate-900 p-3.5 rounded-xl border border-slate-700/80 space-y-2 relative';
      card.innerHTML = `
  ...
  function obtenerDatosFormulario() {
    ...
    return {
      ...
      btnNavColor: getVal('btnNavColor', '#10b981'),
      btnNavText: getVal('btnNavText', 'Pedir por WhatsApp'),
      btnNavIcon: getVal('btnNavIcon', 'fa-brands fa-whatsapp'),
